...
const Pippin = async function (env, sessionId, successcb, failurecb, dcb) {
	if(dcb){
    ...
    if (env == "production") {
        preent = await this.replaceJS(
            "https://sdk.cashfree.com/js/ui/2.0.0/dropinClient.prod.js?v=" +
                Date.now()
        );
    } else {
        preent = await this.replaceJS(
            "https://sdk.cashfree.com/js/ui/2.0.0/dropinClient.sandbox.js?v=" +
                Date.now()
        );
    }
	...
	if (Pippin.isMobile()){
        ...
        let closeBar = document.createElement("div");
        closeBar.classList.add("close-icon");
        closeBar.style.position = "absolute";
        closeBar.style.top = "18px";
        closeBar.style.left="92%";
        closeBar.innerHTML = "<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='white' class='bi bi-x-lg' viewBox='0 0 16 16'><path d='M2.146 2.854a.5.5 0 1 1 .708-.708L8 7.293l5.146-5.147a.5.5 0 0 1 .708.708L8.707 8l5.147 5.146a.5.5 0 0 1-.708.708L8 8.707l-5.146 5.147a.5.5 0 0 1-.708-.708L7.293 8 2.146 2.854Z'/></svg>";
        closeBar.onclick=()=>{
            modalx.close();
        }
        modal.append(closeBar);
    ...
    const dropinConfig = {
        components: ["order-details", "upi", "card", "app", "netbanking", "paylater", "credicardemi", "debitcardemi", "cardlessemi"],
	...
	let cashfree_var = new Cashfree(sessionId);
    cashfree_var.drop(
